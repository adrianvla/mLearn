...
describe('languageDataService', () => {
  ...
  it('discovers dictionary targets from catalog metadata alone (open-ended pairs)', () => {
    const langData = makeLangData({
      languageData: {
        assets: [],
        dictionaryPacks: {
          ru: {
            targetLanguage: 'ru',
            name: 'Russian',
            bundle: { url: 'https://example.com/ja-ru.tar.gz' },
            assets: [],
          },
          // A synthetic third-party display target: no runtime registration
          // exists or is required for the pair to resolve.
          qq: {
            targetLanguage: 'qq',
            name: 'Synthetic Display Language',
            bundle: { url: 'https://example.com/ja-qq.tar.gz' },
            assets: [],
          },
        },
      },
    ...
    // Learning language 'zz' is itself synthetic: resolution must not depend
    // on any known language.
    // The motivating pair shape (ja → ru) resolves from the catalog alone.
    expect(mod.resolveDictionaryTargetLanguage('zz', langData, 'ru')).toBe('ru');
    // Synthetic third-party display target resolves without registration.
    expect(mod.resolveDictionaryTargetLanguage('zz', langData, 'qq')).toBe('qq');
    // Unadvertised pairs stay unresolved — graceful, and never an implicit
    // English fallback.
    expect(mod.resolveDictionaryTargetLanguage('zz', langData, 'zh')).toBeUndefined();
    expect(mod.resolveDictionaryTargetLanguage('zz', langData)).toBeUndefined();
  });
