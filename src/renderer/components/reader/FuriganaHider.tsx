/**
 * Furigana Hider Component
 * Renders white rectangles over detected furigana boxes that fade on hover.
 * This allows learners to test their kanji reading before revealing the furigana.
 */

import { Component, For, Show, createMemo } from 'solid-js';
import type { OcrBox } from './OcrOverlay';
import './FuriganaHider.css';

export interface FuriganaHiderProps {
  /** The furigana boxes to hide (filtered out boxes from OCR) */
  furiganaBoxes: OcrBox[];
...
export const FuriganaHider: Component<FuriganaHiderProps> = (props) => {
  const boxesWithRects = createMemo(() => {
    if (!props.enabled || !props.furiganaBoxes || props.furiganaBoxes.length === 0) {
      return [];
    }
    
    return props.furiganaBoxes.map((box, idx) => ({
      box,
      rect: getBoundingRect(box.box),
      id: `furigana-hider-${idx}`,
    }));
  });
