 ...
 * interval/ease distributions, and forecast data.
 */
...
export const Dashboard: Component = () => {
  ...
  const cardStats = createMemo(() => {
    ...
    for (const card of all) {
      ...
      // Ease & interval distribution (review cards only)
      ...
      addedByMonth.set(monthKey, (addedByMonth.get(monthKey) ?? 0) + 1);
    }
  ...
  const dailyStatsData = createMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const stat = ds[key];
      const dayLabel = i === 0 ? 'Today' : i <= 6 ? d.toLocaleDateString(undefined, { weekday: 'short' }) : `${d.getMonth() + 1}/${d.getDate()}`;

      last30.push({
        label: i % 3 === 0 ? dayLabel : '', // Show every 3rd label
  ...
  const cardStatePie = createMemo((): PieSegment[] => [
    { label: 'Review', value: cardStats().review, color: 'var(--color-success)' },
    { label: 'Learning', value: cardStats().learning, color: 'var(--color-warning)' },
    { label: 'New', value: cardStats().newCards, color: 'var(--color-primary)' },
    { label: 'Suspended', value: cardStats().suspended, color: 'var(--text-tertiary)' },
  ]);

  const maturityPie = createMemo((): PieSegment[] => [
    { label: 'Mature (≥21d)', value: cardStats().matureCount, color: 'var(--color-success)' },
    { label: 'Young (<21d)', value: cardStats().youngCount, color: 'var(--color-info)' },
    { label: 'Learning', value: cardStats().learning, color: 'var(--color-warning)' },
    { label: 'New', value: cardStats().newCards, color: 'var(--color-primary)' },
  ]);

  const wordStatusPie = createMemo((): PieSegment[] => [
    { label: 'Learned', value: wordStats().learned, color: 'var(--color-success)' },
    { label: 'Learning', value: wordStats().learning, color: 'var(--color-warning)' },
    { label: 'Viewed', value: wordStats().unknown, color: 'var(--text-tertiary)' },
  ]);
  ...
  // ── Level breakdown (reused from StatsTab concept) ──
  ...
  return (
    <div class="statistics-dashboard">
      ...
      <div class="dashboard-stats-row">
        <StatCard label="Total Cards" value={cardStats().total} size="md" variant="elevated" />
        <StatCard label="Retention Rate" value={`${cardStats().retentionRate.toFixed(1)}%`} size="md" variant="elevated"
          color={cardStats().retentionRate >= 90 ? 'success' : cardStats().retentionRate >= 80 ? 'warning' : 'error'} />
        <StatCard label="Average Ease" value={formatEase(cardStats().avgEase)} size="md" variant="elevated" />
        <StatCard label="Total Reviews" value={cardStats().totalReviews} size="md" variant="elevated" />
        <StatCard label="Current Streak" value={`${dailyStatsData().streakCurrent}d`} size="md" variant="elevated" color="primary" />
        <StatCard label="Time Watched" value={getTimeWatchedFormatted(t)} size="md" variant="elevated" />
      </div>

      {/* ─── Today's Summary ─── */}
      <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
        <h3 class="dashboard-section-title">Today's Session</h3>
        <div class="dashboard-stats-row compact">
          <StatCard label="Reviews" value={dailyStatsData().todayReviews} size="sm" />
          <StatCard label="New Learned" value={dailyStatsData().todayNew} size="sm" color="success" />
          <StatCard label="Lapses" value={dailyStatsData().todayLapses} size="sm" color={dailyStatsData().todayLapses > 0 ? 'error' : 'default'} />
          <StatCard label="Graduated" value={dailyStatsData().todayGraduated} size="sm" color="success" />
          <StatCard label="Study Time" value={formatDuration(dailyStatsData().todayTime)} size="sm" />
          <StatCard label="Overdue" value={cardStats().overdueCount} size="sm" color={cardStats().overdueCount > 0 ? 'warning' : 'default'} />
        </div>
      </Panel>
      ...
      <div class="dashboard-charts-row">
        <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
          <h3 class="dashboard-section-title">Card States</h3>
          <PieChart
        ...
        <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
          <h3 class="dashboard-section-title">Maturity</h3>
          <PieChart
        ...
        <Show when={wordStats().total > 0}>
          <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
            <h3 class="dashboard-section-title">Word Knowledge</h3>
            <PieChart
      ...
      <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
        <div class="dashboard-section-header">
          <h3 class="dashboard-section-title">Review Activity (Last 30 Days)</h3>
          <div class="dashboard-legend-inline">
            <span class="legend-entry"><span class="legend-dot" style={{ background: 'var(--color-primary)' }} />Reviews</span>
            <span class="legend-entry"><span class="legend-dot" style={{ background: 'var(--color-success)' }} />New</span>
          </div>
        </div>
        <BarChart data={dailyStatsData().last30} height={100} stacked />
      </Panel>

      {/* ─── Distributions Row ─── */}
      <div class="dashboard-charts-row">
        <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
          <h3 class="dashboard-section-title">Interval Distribution</h3>
        ...
        <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
          <h3 class="dashboard-section-title">Ease Factor Distribution</h3>
          <div class="horizontal-bars">
      ...
      {/* ─── Forecast ─── */}
      <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
        <h3 class="dashboard-section-title">14-Day Review Forecast</h3>
        <BarChart data={forecastData()} height={90} />
      </Panel>

      {/* ─── Review Heatmap ─── */}
      <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
        <div class="dashboard-section-header">
          <h3 class="dashboard-section-title">Review Heatmap</h3>
          <div class="dashboard-meta-stats">
            <span>Best streak: {dailyStatsData().streakMax}d</span>
            <span>Days studied: {dailyStatsData().totalDaysStudied}</span>
            <span>Total time: {formatDuration(dailyStatsData().totalStudyTime)}</span>
          </div>
        </div>
      ...
      <Show when={getLanguageFeatures().supportsFrequencyLevels && levelBreakdown().length > 0}>
        <Panel variant="default" rounded="lg" padding="lg" class="dashboard-panel">
          <h3 class="dashboard-section-title">Words by Exam Level</h3>
