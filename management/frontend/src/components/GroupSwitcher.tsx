...
export function GroupSwitcher() {
  ...
  if (scope.groups.length < 2) return <ConsoleButton className="group-switcher" aria-label={`Current group: ${scope.selectedGroup?.name ?? 'None'}`} isDisabled><span>{scope.selectedGroup?.name ?? 'No group'}</span></ConsoleButton>;
