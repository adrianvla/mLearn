...
export function GroupSwitcher() {
  ...
  return <div className="group-switcher"><ConsoleSelect label={`Current group: ${scope.selectedGroup?.name ?? 'None'}`} selectedKey={scope.selectedGroup?.id ?? ''} onSelectionChange={(value) => void scope.selectGroup(value).catch(() => undefined)} options={scope.groups.map((group) => ({ key: group.id, label: group.name }))} /></div>;
}
