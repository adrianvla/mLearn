...
export function AppSidebar({ mobileOpen = false, onNavigate }: { mobileOpen?: boolean; onNavigate?: () => void }) {
  ...
  return <aside className={`app-sidebar ${mobileOpen ? 'is-open' : ''}`}>
    <div className="sidebar-brand"><img src={appLogoUrl} alt="" /><div><strong>mLearn</strong><span>School Console</span></div><ConsoleButton variant="ghost" isIconOnly aria-label="Close navigation" onClick={onNavigate}><X /></ConsoleButton></div>
