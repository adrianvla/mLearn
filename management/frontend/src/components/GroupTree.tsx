...
export function GroupTree({ groups, selectedId, onSelect }: { groups: GroupNode[]; selectedId: string | null; onSelect(id: string): void }) {
  ...
  const render = (group: GroupNode): React.ReactNode => <li key={group.id}><ConsoleButton className={selectedId === group.id ? 'selected' : ''} onClick={() => onSelect(group.id)}>{group.name}<small>{group.status}</small></ConsoleButton>{groups.some((child) => child.parentId === group.id) && <ul>{groups.filter((child) => child.parentId === group.id).map(render)}</ul>}</li>;
