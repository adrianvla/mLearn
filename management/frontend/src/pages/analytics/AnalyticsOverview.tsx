...
export function AnalyticsOverview({ summary, history, comparison, visibleMetrics, onVisibleMetricsChange, activityError, llm, llmError, blocks, policyError, onBucketClick }: AnalyticsOverviewProps) {
  ...
  return <div className="analytics-overview">
    ...
    <section className="analytics-panel" aria-labelledby="policy-blocks-heading">
      ...
      {policyError ? <PanelError title="Policy blocks" message={policyError} /> : blocks === null || history === null ? <PanelLoading title="Policy blocks" /> : <><div className="metric-grid"><MetricCard label="Blocked requests" value={blocks.blocks} detail="Requests rejected before provider execution" /></div><HistoricalChart title="Policy blocks" series={policySeries} timezone={history.timezone} /></>}
    </section>
  </div>;
}
