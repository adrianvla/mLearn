...
function UserActivityHistory({ activity, error }: { activity: UserDailyHistory | null; error: string | null }) {
  ...
  const membershipChanges = activity?.membershipChanges ?? [];
  const noRecordedValues = activity !== null && activity.daily.every((day) => day.values === null);
  return <section aria-labelledby="user-activity-heading"><h3 id="user-activity-heading">Recorded daily activity</h3><p>Sessions, pages, video time, flashcard sessions, LLM usage, cost, and policy blocks recorded in the selected group scope.</p>{error !== null ? <p role="alert">Unable to load daily activity. {error}</p> : null}{activity === null && error === null ? <p role="status">Loading daily activity.</p> : null}{noRecordedValues ? <p role="status">No daily activity was recorded in this period. Coverage remains explicit below.</p> : null}{series.length > 0 ? <><HistoricalChart title="Daily user activity chart" series={series} timezone={activity?.timezone} /><AnalyticsHistoryTable title="Daily user activity" series={series} timezone={activity?.timezone} /></> : null}{activity !== null ? <><section aria-labelledby="user-device-history-heading"><h4 id="user-device-history-heading">Recorded devices</h4>{devices.length === 0 ? <p role="status">No devices were recorded in this period.</p> : <div className="table-scroll"><table aria-label="User device history"><thead><tr><th>Device</th><th>Platform</th><th>First recorded</th><th>Last seen</th></tr></thead><tbody>{devices.map((device) => <tr key={device.id}><th>{device.name}</th><td>{device.platform}</td><td>{new Intl.DateTimeFormat(undefined, { timeZone: activity.timezone, dateStyle: 'medium', timeStyle: 'short' }).format(new Date(device.firstRecordedAt * 1000))}</td><td>{new Intl.DateTimeFormat(undefined, { timeZone: activity.timezone, dateStyle: 'medium', timeStyle: 'short' }).format(new Date(device.lastSeenAt * 1000))}</td></tr>)}</tbody></table></div>}</section><section aria-labelledby="user-membership-history-heading"><h4 id="user-membership-history-heading">Membership changes</h4>{membershipChanges.length === 0 ? <p role="status">No membership changes were recorded in this period.</p> : <div className="table-scroll"><table aria-label="User membership change history"><thead><tr><th>Action</th><th>Group</th><th>Recorded</th></tr></thead><tbody>{membershipChanges.map((change) => <tr key={`${change.membershipId}:${change.occurredAt}`}><th>{change.action}</th><td>{change.groupName}</td><td>{new Intl.DateTimeFormat(undefined, { timeZone: activity.timezone, dateStyle: 'medium', timeStyle: 'short' }).format(new Date(change.occurredAt * 1000))}</td></tr>)}</tbody></table></div>}</section></> : null}</section>;
}
