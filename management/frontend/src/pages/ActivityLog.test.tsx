...
it('loads scoped audit events with safe filters and opens redacted event details', async () => {
  const fetchMock = vi.fn(async () => response({
    events: [{
      id: 'event-1', actor: 'teacher-1', action: 'policy.published',
      targetType: 'policy', targetId: 'reading', authorizedGroupId: 'german',
  ...
  fireEvent.click(await screen.findByRole('option', { name: 'Policy published' }));
  fireEvent.click(screen.getByRole('button', { name: /Target type$/ }));
  fireEvent.click(await screen.findByRole('option', { name: 'Policy' }));

  await waitFor(() => expect(fetchMock).toHaveBeenLastCalledWith(
    expect.stringContaining('groupId=german&actorUserId=teacher-1&action=policy.published&targetType=policy&targetId=reading'),
    expect.objectContaining({ signal: expect.any(AbortSignal) }),
  ));

  fireEvent.click(screen.getByRole('button', { name: 'policy.published' }));
