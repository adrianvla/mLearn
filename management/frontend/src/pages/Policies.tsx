...
type Setting = {
  key: string;
  valueType: "boolean" | "number" | "string" | "stringOrNull" | "select";
  allowedValues: string[];
};
type Version = { id: string; summary: string; createdAt: number };
const empty = (): Json => ({
  settings: {},
  features: {},
  llm: { quotas: [] },
...
export default function Policies() {
  ...
  const selected =
    collection.local.find((policy) => policy.id === selectedId) ?? null;
  const dirty = useMemo(
  ...
  const nextStep = !selected
    ? "Select a policy"
    : dirty || !savedHash
      ? "Save draft before validating or publishing"
      : validatedHash !== savedHash
        ? "Validate this draft before publishing"
        : !summary.trim()
          ? "Add a publish summary"
          : null;
  const refresh = async (preserve = true) => {
    ...
    setSelectedId((previous) =>
      preserve && result.local.some((policy) => policy.id === previous)
        ? previous
        : (result.local[0]?.id ?? null),
    );
  };
  useEffect(() => {
    if (!groupId) return;
    let cancelled = false;
    Promise.all([refresh(false), api.get<Setting[]>("/api/policy-registry")])
      .then(([, settings]) => {
        if (!cancelled) setRegistry(settings);
      })
      .catch((reason) => !cancelled && setError(text(reason)));
    return () => {
      cancelled = true;
    };
  }, [groupId]);
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get<Draft | null>(`/api/policies/${selectedId}/draft`)
      .then((result) => {
        if (cancelled) return;
        const document = result?.document ?? empty();
        setDraft(document);
        setSaved(document);
        setSavedHash(result?.documentHash ?? null);
        setValidatedHash(null);
      })
      .catch((reason) => !cancelled && setError(text(reason)));
    return () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get<{ items: Version[] }>(`/api/policies/${selectedId}/history`)
      .then((result) => {
  ...
  const create = async () => {
    ...
    setError(null);
    try {
      const policy = await api.get<Policy>(`/api/groups/${groupId}/policies`, {
        method: "POST",
  ...
  const save = async () => {
    ...
    try {
      const result = await api.get<Draft>(
        `/api/policies/${selected.id}/draft`,
        {
          method: "PUT",
          body: JSON.stringify({
            document: draft,
            expectedDocumentHash: savedHash,
          }),
        },
      );
      setSaved(result.document);
      setDraft(result.document);
      setSavedHash(result.documentHash);
      setValidatedHash(null);
      await refresh();
    } catch (reason) {
      setError(text(reason));
  ...
  const validate = async () => {
    ...
    setError(null);
    try {
      const result = await api.get<{ documentHash: string }>(
        `/api/policies/${selected.id}/validate`,
        { method: "POST" },
      );
      setValidatedHash(result.documentHash);
    } catch (reason) {
      setError(text(reason));
    } finally {
      setBusy(false);
  ...
  const publish = async () => {
    ...
    setError(null);
    try {
      await api.get(`/api/policies/${selected.id}/publish`, {
        method: "POST",
  ...
  const addRule = () => {
    if (ruleKind === "setting" && settingKey) {
      const entry = registry.find((setting) => setting.key === settingKey);
      setDraft((current) => ({
        ...current,
        settings: {
          ...current.settings,
          [settingKey]: { value: initial(entry), locked: true },
        },
      }));
    }
    if (ruleKind === "llm")
      setDraft((current) => ({
        ...
        llm: {
          ...current.llm,
          enabled: true,
          quotas: current.llm?.quotas ?? [],
        },
      }));
    if (ruleKind === "retention")
      setDraft((current) => ({
        ...current,
        governance: {
          ...current.governance,
  ...
  return (
    <div className="resource-page">
      <PageToolbar
        title={group ? `Policies for ${group.name}` : "Policies"}
        description="Create named policies, add only the rules you need, then save, validate, and publish."
      />
      ...
      <div className="policy-workspace">
        ...
        <main className="policy-builder">
          {!selected ? (
            ...
            <>
              <header className="policy-builder-header">
                <div>
                  <h2>{selected.name}</h2>
                  <p>
                    {dirty
                      ? "Unsaved changes"
                      : validatedHash === savedHash && savedHash
                        ? "Validated"
              ...
              <section className="policy-rules">
                <h3>Rules</h3>
                {Object.entries(draft.settings ?? {}).map(
                  ([key, rule]: [string, any]) => (
                    <SettingCard
                      key={key}
                      entry={registry.find((setting) => setting.key === key)}
                      name={key}
                      rule={rule}
                      disabled={!editable}
                      onChange={(value) =>
                        setDraft((current) => ({
                          ...
                          settings: {
                            ...current.settings,
                            [key]: { ...current.settings[key], value },
                          },
                        }))
                      }
                      onRemove={() =>
                ...
                {draft.llm?.enabled !== undefined && (
                  <RuleCard
                    title="Enable LLM access"
                    onRemove={() =>
                      setDraft((current) => ({
                        ...current,
                        llm: { ...current.llm, enabled: undefined },
                      }))
                    }
                  >
                    <input
                  ...
                  <RuleCard
                    ...
                    onRemove={() =>
                      setDraft((current) => ({
                        ...
                        governance: {
                          ...current.governance,
                          activityRetentionDays: undefined,
                ...
                {!Object.keys(draft.settings ?? {}).length &&
                  ...
                  draft.governance?.activityRetentionDays === undefined && (
                    <p>
                      No rules yet. Add a rule to define what this policy
                      enforces.
                    </p>
                  )}
              </section>
              <section className="policy-history">
                ...
                {history.length ? history.map((version) => (
                  <div key={version.id}>
                    ...
                    <span>{new Date(version.createdAt * 1000).toLocaleString()}</span>
                  </div>
...
function initial(entry?: Setting): unknown {
  if (entry?.valueType === "boolean") return false;
  if (entry?.valueType === "number") return 0;
  if (entry?.valueType === "select") return entry.allowedValues[0] ?? "";
  return "";
}
...
function text(reason: unknown) {
  return reason instanceof Error ? reason.message : "Policy request failed";
}
