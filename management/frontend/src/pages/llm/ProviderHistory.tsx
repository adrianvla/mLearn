  ...
  return <ConsoleDialog open={open} onOpenChange={onOpenChange} title={`${providerName ?? 'Provider'} history`}>
    ...
    {history !== null ? <>
      ...
      <section aria-labelledby="provider-health-heading">
        ...
        {history.healthChecks.length === 0 ? <p role="status">No provider configuration checks were recorded in this period.</p> : null}<div className="table-scroll"><table aria-label="Provider health history"><caption className="sr-only">Provider health history</caption><thead><tr><th scope="col">Recorded</th><th scope="col">Actor</th><th scope="col">Configuration valid</th><th scope="col">Network check performed</th><th scope="col">Outcome</th></tr></thead><tbody>{history.healthChecks.map((check) => <tr key={`${check.createdAt}-${check.actorUserId}-${check.outcome}`}><td>{new Date(check.createdAt * 1_000).toLocaleString()}</td><td>{check.actorUserId}</td><td>{check.configurationValid ? 'Yes' : 'No'}</td><td>{check.networkCheckPerformed ? 'Yes' : 'No'}</td><td>{check.outcome}</td></tr>)}</tbody></table></div>
      </section>
    </> : null}
  </ConsoleDialog>;
}
