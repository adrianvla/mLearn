...
const pythonFound = () => {
    ...
    const command = ["\"" + pythonExecutable + "\"", "\"" + path.join(resPath, 'server.py') + "\"", String(settings.ankiConnectUrl), String(settings.use_anki), String(settings.language), "\"" + String(resPath) + "\"", llmEnabled ? 'true' : 'false', ocrEnabled ? 'true' : 'false', "\"" + app.getPath('userData') + "\""].join(" ");
        console.log("WINDOWS:::",command)
        pythonChildProcess = exec(command);
    }else{
    pythonChildProcess = spawn('env', [pythonExecutable, path.join(resPath, 'server.py'), String(settings.ankiConnectUrl), String(settings.use_anki), String(settings.language), String(resPath), llmEnabled ? 'true' : 'false', ocrEnabled ? 'true' : 'false', app.getPath('userData')], {
            env: process.env
        });
    }
